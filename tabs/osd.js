...
OSD.use_191_api = function() {
    return semver.gte(CONFIG.flightControllerVersion, '1.9.1');
};
...
OSD.reload = function(callback) {
    ...
    if (OSD.use_191_api()) {
        MSP.promise(MSPCodes.MSP2_INAV_OSD_LAYOUTS).then(function (resp) {
            ...
            OSD.update_selected_layout(OSD.data.selected_layout || 0);
...
OSD.update_selected_layout = function(new_layout) {
...
OSD.save_alarms = function(callback) {
    // Before 1.9.1, config and alarms are saved with
    // the same MSP cmd.
    if (!OSD.use_191_api()) {
        return OSD.save_config(callback);
    }
    var data = OSD.msp.encodeAlarms();
    return MSP.promise(MSPCodes.MSP2_INAV_OSD_SET_ALARMS, data).then(callback);
}

OSD.save_config = function(callback) {
    if (OSD.use_191_api()) {
...
OSD.save_item = function(item, callback) {
...
OSD.GUI.update_video_mode = function() {
    // video mode
...
OSD.GUI.update_alarms = function() {
    // alarms
...
OSD.GUI.update_fields = function() {
...
OSD.GUI.update_previews = function() {
    // buffer the preview;
    OSD.data.preview = [];
...
OSD.GUI.update_all = function() {
    ...
    if (OSD.data.layout_count > 1) {
        ...
        layouts.on('change', function() {
            OSD.update_selected_layout(parseInt(layouts.val()));
            OSD.GUI.update_fields();
            OSD.GUI.update_previews();
        });
    } else {
        layouts.hide();
        layouts.off('change');
    ...
    $('.supported').fadeIn();
    OSD.GUI.update_video_mode();
    OSD.GUI.update_units();
    OSD.GUI.update_alarms();
    OSD.GUI.update_fields();
    OSD.GUI.update_previews();
};
...
OSD.GUI.save_item = function(item) {
    OSD.save_item(item, function() {
        OSD.GUI.update_previews();
    });
};

OSD.GUI.save_config = function() {
    OSD.save_config(function() {
        OSD.GUI.update_previews();
        OSD.GUI.update_alarms();
    });
};
